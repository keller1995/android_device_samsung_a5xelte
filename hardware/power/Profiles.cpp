...
void Profiles::loadProfileImpl(SecPowerProfile *profile, xmlXPathContext *ctx, const char *path) {
    ...
    profile->cpu.cluster1.enabled       = XML_GET_BOOL("cpu/cluster1/enabled");
    profile->cpu.cluster1.cores.enabled = XML_GET_BOOL("cpu/cluster1/cores/enabled");
    profile->cpu.cluster2.enabled        = XML_GET_BOOL("cpu/cluster2/enabled");
    profile->cpu.cluster2.cores.enabled  = XML_GET_BOOL("cpu/cluster2/cores/enabled");
    ...
    if (profile->cpu.enabled) {
        if (profile->cpu.cluster1.enabled) {
            ...
            XML_GET_CPUGOV_DATA(cluster1);
        }

        if (profile->cpu.cluster2.enabled) {
            XML_GET_CPUGOV(cluster2);
            XML_GET_CPUGOV_DATA(cluster2);
        }
    }
    ...
    if (profile->gpu.enabled) {
        if (profile->gpu.dvfs.enabled) {
            profile->gpu.dvfs.min_lock = XML_GET_UINT("gpu/dvfs/min_lock");
            profile->gpu.dvfs.max_lock = XML_GET_UINT("gpu/dvfs/max_lock");
        }
